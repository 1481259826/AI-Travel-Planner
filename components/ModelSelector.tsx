'use client'

import { AIModel, AIModelConfig } from '@/types'
import { availableModels, getModelById } from '@/lib/models'

interface ModelSelectorProps {
  selectedModel: AIModel
  onModelChange: (model: AIModel) => void
  className?: string
}

export default function ModelSelector({ selectedModel, onModelChange, className = '' }: ModelSelectorProps) {
  const currentModel = getModelById(selectedModel)

  return (
    <div className={`space-y-2 ${className}`}>
      <label htmlFor="model-select" className="text-sm font-medium text-gray-700">
        AI 模型选择
      </label>
      <select
        id="model-select"
        value={selectedModel}
        onChange={(e) => onModelChange(e.target.value as AIModel)}
        className="flex h-10 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent disabled:cursor-not-allowed disabled:opacity-50"
      >
        {availableModels.map((model: AIModelConfig) => (
          <option
            key={model.id}
            value={model.id}
            disabled={!model.enabled}
          >
            {model.name} - {model.description}
          </option>
        ))}
      </select>

      {/* 显示当前选中模型的额外信息 */}
      {currentModel && (
        <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
          <span className="capitalize font-medium">{currentModel.provider}</span>
          <span>•</span>
          <span>最大 {currentModel.maxTokens.toLocaleString()} tokens</span>
        </div>
      )}
    </div>
  )
}
