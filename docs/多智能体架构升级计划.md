...
#### 任务清单
- [ ] 1.1 创建 MCP Server 项目结构 (`mcp-servers/amap/`)
- [ ] 1.2 实现天气查询工具 (`get_weather_forecast`)
- [ ] 1.3 实现 POI 搜索工具 (`search_poi`, `search_nearby`)
- [ ] 1.4 实现路线规划工具 (`get_driving_route`, `get_transit_route`, `get_walking_route`)
- [ ] 1.5 实现地理编码工具 (`geocode`, `reverse_geocode`)
- [ ] 1.6 实现距离计算工具 (`calculate_distance`)
- [ ] 1.7 编写 MCP Server 单元测试
- [ ] 1.8 配置 SSE/stdio 两种连接模式
- [ ] 1.9 编写 MCP Server 文档
...
#### 任务清单
- [ ] 2.1 安装 LangGraph 依赖 (`@langchain/langgraph`, `@langchain/core`)
- [ ] 2.2 定义 TripState Annotation Schema (`lib/agents/state.ts`)
- [ ] 2.3 创建 StateGraph 工作流 (`lib/agents/workflow.ts`)
- [ ] 2.4 实现 MCP 客户端封装 (`lib/agents/mcp-client.ts`)
- [ ] 2.5 实现简单的线性流程 (Weather → Planner → End)
- [ ] 2.6 添加条件边和循环边 (Budget 重试)
- [ ] 2.7 实现并行节点 (Resource Agents Fan-out/Fan-in)
- [ ] 2.8 配置 Checkpointer 检查点存储
- [ ] 2.9 编写工作流单元测试
...
#### 任务清单
- [ ] 3.1 实现 Weather Scout Agent (`lib/agents/nodes/weather-scout.ts`)
- [ ] 3.2 实现 Itinerary Planner Agent (`lib/agents/nodes/itinerary-planner.ts`)
- [ ] 3.3 实现 Accommodation Specialist Agent (`lib/agents/nodes/accommodation.ts`)
- [ ] 3.4 实现 Transport Logistician Agent (`lib/agents/nodes/transport.ts`)
- [ ] 3.5 实现 Dining Recommender Agent (`lib/agents/nodes/dining.ts`)
- [ ] 3.6 实现 Budget Critic Agent (`lib/agents/nodes/budget-critic.ts`)
- [ ] 3.7 实现 Finalize Agent (汇总输出) (`lib/agents/nodes/finalize.ts`)
- [ ] 3.8 编写各 Agent 的 System Prompts (`lib/agents/prompts/`)
- [ ] 3.9 编写各 Agent 单元测试
...
#### 任务清单
- [ ] 4.1 创建新 API 端点 (`/api/v2/generate-itinerary`)
- [ ] 4.2 集成 LangGraph 工作流到 API Route
- [ ] 4.3 实现 Feature Flag 切换 (`USE_LANGGRAPH`)
- [ ] 4.4 实现 SSE 流式响应 (使用 `app.stream()`)
- [ ] 4.5 更新前端调用逻辑，支持进度显示
- [ ] 4.6 添加 Agent 执行进度 UI 组件
- [ ] 4.7 编写集成测试
- [ ] 4.8 性能对比测试 (单体 vs LangGraph)
...
|------|------|----------|
| 2.1 安装 LangGraph 依赖 | [ ] | - |
| 2.2 定义 TripState Annotation | [ ] | - |
| 2.3 创建 StateGraph 工作流 | [ ] | - |
| 2.4 实现 MCP 客户端封装 | [ ] | - |
| 2.5 实现线性流程 | [ ] | - |
| 2.6 添加条件边和循环边 | [ ] | - |
| 2.7 实现并行节点 | [ ] | - |
...
| 任务 | 状态 | 完成日期 |
|------|------|----------|
| 3.1 Weather Scout Agent | [ ] | - |
| 3.2 Itinerary Planner Agent | [ ] | - |
| 3.3 Accommodation Agent | [ ] | - |
| 3.4 Transport Agent | [ ] | - |
| 3.5 Dining Agent | [ ] | - |
| 3.6 Budget Critic Agent | [ ] | - |
| 3.7 Finalize Agent | [ ] | - |
| 3.8 编写 Agent Prompts | [ ] | - |
| 3.9 编写 Agent 测试 | [ ] | - |
