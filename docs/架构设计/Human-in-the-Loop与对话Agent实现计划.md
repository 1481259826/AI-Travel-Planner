...
| # | 任务 | 描述 | 状态 |
|---|------|------|------|
| 1.1 | 数据库迁移 | 创建 `workflow_interrupts` 表 | ⬜ 待开始 |
| 1.2 | 状态扩展 | 实现 `HITLTripStateAnnotation` | ⬜ 待开始 |
| 1.3 | 工作流改造 | 实现 `createHITLWorkflow` | ⬜ 待开始 |
| 1.4 | API 开发 | 实现 `/api/v2/workflow/*` 端点 | ⬜ 待开始 |
| 1.5 | SSE 扩展 | 支持 interrupt 事件类型 | ⬜ 待开始 |
...
> 📌 **下一步**: 开始 Phase 1 - Human-in-the-Loop 基础实现
