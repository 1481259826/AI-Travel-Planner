...
- [ ] 添加 `ItineraryModificationOperation` 枚举
- [ ] 添加 `ModificationPreview` 接口
- [ ] 添加 `ModificationChange` 接口
- [ ] 添加 `DayPlanSummary` 接口
- [ ] 添加 `PrepareItineraryModificationParams` 接口
- [ ] 添加 `ConfirmItineraryModificationParams` 接口

#### 1.2 工具定义 (`lib/chat/tools.ts`)

- [ ] 添加 `prepare_itinerary_modification` 工具定义
- [ ] 添加 `confirm_itinerary_modification` 工具定义
- [ ] 更新 `TOOL_DESCRIPTIONS` 和 `TOOL_ICONS`
...
- [ ] 实现 `prepareItineraryModification()` 方法
  - 加载行程
  - 计算修改后的行程
  - 生成 before/after 对比
  - 生成变更列表
  ...
  - 缓存 pending 修改
  - 返回预览数据
- [ ] 实现 `confirmItineraryModification()` 方法
  - 从缓存获取 pending 修改
  - 应用到数据库
  - 清除缓存
  - 返回更新结果
...
- [ ] 创建 Zustand store
- [ ] `pendingModification` 状态
- [ ] `setPendingModification()` 方法
- [ ] `confirmModification()` 方法
- [ ] `cancelModification()` 方法
...
- [ ] 识别 `modification_preview` 类型工具结果
- [ ] 渲染 ModificationPreviewCard

#### 1.8 Hook 扩展 (`hooks/useChatAgent.ts`)

- [ ] 处理 `prepare_itinerary_modification` 工具结果
- [ ] 更新 modification store
- [ ] 实现确认/取消回调
- [ ] 处理确认后的行程刷新
