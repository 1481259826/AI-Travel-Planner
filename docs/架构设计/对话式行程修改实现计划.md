# 对话式行程修改功能实现计划

> 版本: v1.3
> 创建日期: 2025-12-08
> 最后更新: 2025-12-10
> 状态: Phase 3 已完成
> 前置文档: [对话式行程生成实现计划.md](./对话式行程生成实现计划.md)

---

## 一、需求概述

### 1.1 目标

在现有对话系统中增强行程修改功能，让用户通过自然语言对话来修改已生成的行程，支持从简单的景点增删到智能重规划的完整修改场景。

### 1.2 核心流程

```
用户描述修改需求 → AI 解析意图 → 生成修改预览（前后对比）
    → 用户确认/取消/微调 → 保存到数据库 → 刷新行程显示
```

### 1.3 功能范围

| 功能类别 | 具体操作 | 复杂度 |
|---------|---------|--------|
| **基础修改** | 添加/删除景点、调整时间、重新排序、更换住宿/餐厅 | 低 |
| **结构变更** | 增加/删除天数、拆分/合并天数 | 中 |
| **智能重规划** | 优化路线顺序、根据约束重新规划某天、天气适应 | 高 |
| **完整重生成** | 重新生成某一天或连续多天的行程 | 很高 |

### 1.4 使用场景

| 入口 | 场景描述 |
|-----|---------|
| **行程详情页** | 用户查看行程时，通过侧边栏或悬浮按钮打开对话面板，直接修改当前行程 |
| **对话助手页** | 在 `/dashboard/chat` 页面，选择要修改的行程后进行对话 |

### 1.5 确认机制

采用**预览确认**模式：
- 所有修改先生成预览，显示变更摘要和前后对比
- 用户确认后才保存到数据库
- 支持取消或微调预览内容

---

## 二、现有系统分析

### 2.1 现有 modify_itinerary 工具

**位置**: `lib/chat/tools.ts` (第 132-200 行)

**现有操作类型**:
```typescript
enum: [
  'add_attraction',    // 添加景点
  'remove_attraction', // 删除景点
  'reorder',           // 调整顺序
  'change_time',       // 修改时间
  'add_day',           // 增加一天
  'remove_day',        // 删除一天
  'change_hotel',      // 更换酒店
  'change_restaurant', // 更换餐厅
]
```

**问题**:
- ⚠️ 直接保存到数据库，无预览确认机制
- ⚠️ 缺少智能重规划功能
- ⚠️ 不支持部分重生成

### 2.2 现有执行器实现

**位置**: `lib/chat/executor.ts` (第 400-522 行)

`modifyItinerary()` 方法已实现：
- 景点增删
- 顺序调整
- 时间修改
- 天数增删

**问题**: 直接调用 `supabase.update()` 保存，无法预览

### 2.3 现有对话式生成组件

已有可复用的组件：
- `TripFormCard.tsx` - 表单预览卡片（可参考设计）
- `TripGenerationProgress.tsx` - 进度显示（重规划时可复用）
- `TripCompletionCard.tsx` - 完成卡片

---

## 三、设计方案

### 3.1 工具设计：两步确认流程

为了支持预览确认，设计两个新工具替代现有的直接保存模式：

#### 工具 1: `prepare_itinerary_modification`

- **职责**: 准备修改，生成预览数据，**不保存**
- **触发时机**: AI 识别到用户想要修改行程时
- **返回**: 修改预览（before/after 对比）+ 修改 ID + 影响评估

#### 工具 2: `confirm_itinerary_modification`

- **职责**: 用户确认后，应用修改并保存到数据库
- **触发时机**: 用户点击"确认修改"按钮
- **返回**: 保存结果 + 更新后的行程

### 3.2 操作类型扩展

```
┌─────────────────────────────────────────────────────────────────┐
│                      操作类型分层架构                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Level 1: 基础修改（直接计算预览，毫秒级）                        │
│  ├── add_attraction      添加景点到指定天数                      │
│  ├── remove_attraction   从指定天数删除景点                      │
│  ├── reorder            调整景点顺序（同天/跨天）                 │
│  ├── change_time        修改景点开始时间                         │
│  ├── change_hotel       更换住宿                                 │
│  └── change_restaurant  更换餐厅                                 │
│                                                                  │
│  Level 2: 结构变更（需要重新计算时间分配，秒级）                  │
│  ├── add_day            在指定位置插入新的一天                   │
│  ├── remove_day         删除指定天数                             │
│  ├── split_day          将一天拆分为两天                         │
│  └── merge_days         将连续两天合并为一天                     │
│                                                                  │
│  Level 3: 智能重规划（调用 AI 优化，10秒级）                     │
│  ├── optimize_route     优化路线顺序，最小化交通时间              │
│  ├── replan_day         根据约束重新规划某天（如景点关闭）        │
│  └── adjust_for_weather 根据天气调整行程（室内/室外）             │
│                                                                  │
│  Level 4: 完整重生成（调用 LangGraph 工作流，分钟级）            │
│  ├── regenerate_day          重新生成某天的完整行程               │
│  └── regenerate_trip_segment 重新生成连续多天的行程               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 3.3 确认界面：修改预览卡片

```
┌────────────────────────────────────────────────────────────────┐
│  ✏️ 行程修改预览                              [删除景点]       │
├────────────────────────────────────────────────────────────────┤
│                                                                 │
│  📍 变更摘要                                                    │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ ❌ 删除「西湖游船」从第 2 天                              │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  📊 影响评估                                                    │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ • 受影响天数: 第 2 天                                     │   │
│  │ • 预算变化: -¥120                                         │   │
│  │ • 时间变化: 释放 2 小时                                   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ⚠️ 删除后第 2 天下午将有较多空闲时间，建议添加其他活动        │
│                                                                 │
├────────────────────────────────────────────────────────────────┤
│              [取消]    [查看详情]    [确认修改]                 │
└────────────────────────────────────────────────────────────────┘
```

### 3.4 详细对比视图（点击"查看详情"展开）

```
┌────────────────────────────────────────────────────────────────┐
│  📋 修改前后对比                                               │
├───────────────────────────┬────────────────────────────────────┤
│         修改前            │            修改后                  │
├───────────────────────────┼────────────────────────────────────┤
│  第 2 天 - 12月26日       │  第 2 天 - 12月26日                │
│  ├─ 09:00 灵隐寺          │  ├─ 09:00 灵隐寺                   │
│  ├─ 12:00 午餐            │  ├─ 12:00 午餐                     │
│  ├─ 14:00 西湖游船 ❌     │  ├─ 14:00 (空闲)                   │
│  └─ 17:00 河坊街          │  └─ 17:00 河坊街                   │
│                           │                                    │
│  预算: ¥1,200             │  预算: ¥1,080 (-¥120)              │
└───────────────────────────┴────────────────────────────────────┘
```

---

## 四、数据流架构

### 4.1 基础修改流程

```
用户消息: "把第二天的西湖游船删掉"
                │
                ▼
        ┌───────────────┐
        │ TravelChat    │  1. 解析用户意图
        │ Agent         │  2. 提取参数:
        └───────────────┘     - operation: remove_attraction
                │             - day_index: 1
                ▼             - activity_name: "西湖游船"
┌─────────────────────────────────────┐
│ prepare_itinerary_modification      │
│ 工具调用                             │
└─────────────────────────────────────┘
                │
                ▼
        ┌───────────────┐
        │ ToolExecutor  │  1. 加载当前行程
        │               │  2. 定位"西湖游船"
        └───────────────┘  3. 计算删除后的行程
                │          4. 生成 before/after 对比
                ▼          5. 缓存 pending 修改
┌─────────────────────────────────────┐
│ ModificationPreview 响应             │
│ {                                   │
│   id: "mod_abc123",                 │
│   operation: "remove_attraction",   │
│   before: { days: [...] },          │
│   after: { days: [...] },           │
│   changes: [{ type: "remove", ... }]│
│   impact: { costDelta: -120, ... }  │
│ }                                   │
└─────────────────────────────────────┘
                │
                ▼
┌─────────────────────────────────────┐
│ 前端渲染 ModificationPreviewCard    │
│ 显示变更摘要 + 影响评估              │
└─────────────────────────────────────┘
                │
        用户点击 [确认修改]
                │
                ▼
┌─────────────────────────────────────┐
│ confirm_itinerary_modification      │
│ { modification_id: "mod_abc123" }   │
└─────────────────────────────────────┘
                │
                ▼
        ┌───────────────┐
        │ ToolExecutor  │  1. 从缓存获取 pending 修改
        │               │  2. 应用到数据库
        └───────────────┘  3. 清除缓存
                │          4. 返回更新后的行程
                ▼
┌─────────────────────────────────────┐
│ 完成响应                             │
│ "已从第 2 天删除「西湖游船」"         │
│ + 更新后的行程数据                   │
└─────────────────────────────────────┘
                │
                ▼
┌─────────────────────────────────────┐
│ 前端刷新行程显示                     │
│ (如果在行程详情页，滚动到修改位置)    │
└─────────────────────────────────────┘
```

### 4.2 智能重规划流程

```
用户消息: "第二天的博物馆临时闭馆了，帮我重新规划这一天"
                │
                ▼
        ┌───────────────┐
        │ TravelChat    │  解析意图:
        │ Agent         │  - operation: replan_day
        └───────────────┘  - day_index: 1
                │          - reason: "博物馆闭馆"
                ▼
┌─────────────────────────────────────┐
│ prepare_itinerary_modification      │
│ { operation: "replan_day", ... }    │
└─────────────────────────────────────┘
                │
                ▼
        ┌───────────────┐
        │ ToolExecutor  │  1. 加载行程和第 2 天数据
        │               │  2. 提取约束条件
        └───────────────┘     - 排除: 博物馆
                │             - 保留: 已确定的餐厅
                ▼             - 偏好: 原有的旅行风格
┌─────────────────────────────────────┐
│ Modification Workflow               │
│ (调用 LangGraph 部分 Agent)          │
│                                     │
│  ┌─────────────┐  ┌─────────────┐   │
│  │ Itinerary   │→ │ Transport   │   │
│  │ Planner     │  │ Agent       │   │
│  └─────────────┘  └─────────────┘   │
│         │                │          │
│         ▼                ▼          │
│  ┌─────────────┐                    │
│  │ Dining      │                    │
│  │ Agent       │                    │
│  └─────────────┘                    │
└─────────────────────────────────────┘
                │
                ▼ 返回新的第 2 天计划
┌─────────────────────────────────────┐
│ ModificationPreview 响应             │
│ 包含新的景点推荐、时间安排、餐饮推荐  │
└─────────────────────────────────────┘
                │
        (后续流程同上)
```

---

## 五、类型定义

### 5.1 操作类型枚举

```typescript
// lib/chat/types.ts

/**
 * 行程修改操作类型
 */
export type ItineraryModificationOperation =
  // 基础修改
  | 'add_attraction'
  | 'remove_attraction'
  | 'reorder'
  | 'change_time'
  | 'change_hotel'
  | 'change_restaurant'
  // 结构变更
  | 'add_day'
  | 'remove_day'
  | 'split_day'
  | 'merge_days'
  // 智能重规划
  | 'optimize_route'
  | 'replan_day'
  | 'adjust_for_weather'
  // 完整重生成
  | 'regenerate_day'
  | 'regenerate_trip_segment'
```

### 5.2 修改预览数据结构

```typescript
/**
 * 修改预览数据
 */
export interface ModificationPreview {
  /** 唯一修改 ID，用于确认时引用 */
  id: string

  /** 行程 ID */
  tripId: string

  /** 操作类型 */
  operation: ItineraryModificationOperation

  /** 修改前的行程摘要 */
  before: {
    days: DayPlanSummary[]
    totalCost: number
  }

  /** 修改后的行程摘要 */
  after: {
    days: DayPlanSummary[]
    totalCost: number
  }

  /** 变更列表 */
  changes: ModificationChange[]

  /** 影响评估 */
  impact: {
    /** 受影响的天数 */
    affectedDays: number[]
    /** 成本变化（正数表示增加） */
    costDelta: number
    /** 时间影响描述 */
    timeImpact: string
    /** 警告信息 */
    warnings: string[]
  }

  /** 创建时间 */
  createdAt: number

  /** 过期时间（10分钟后） */
  expiresAt: number

  /** 状态 */
  status: 'pending' | 'confirmed' | 'cancelled' | 'expired'
}

/**
 * 单个变更项
 */
export interface ModificationChange {
  /** 变更类型 */
  type: 'add' | 'remove' | 'modify' | 'reorder'

  /** 天数索引 */
  dayIndex: number

  /** 项目类型 */
  itemType: 'attraction' | 'meal' | 'hotel' | 'transport'

  /** 项目名称 */
  itemName: string

  /** 变更描述 */
  description: string

  /** 变更前的值（可选） */
  before?: unknown

  /** 变更后的值（可选） */
  after?: unknown
}

/**
 * 天计划摘要（用于预览显示）
 */
export interface DayPlanSummary {
  day: number
  date: string
  activities: Array<{
    time: string
    name: string
    type: string
    /** 是否有变更 */
    isChanged?: boolean
    /** 变更类型 */
    changeType?: 'added' | 'removed' | 'modified'
  }>
}
```

### 5.3 工具参数类型

```typescript
/**
 * prepare_itinerary_modification 工具参数
 */
export interface PrepareItineraryModificationParams {
  /** 行程 ID */
  trip_id: string

  /** 操作类型 */
  operation: ItineraryModificationOperation

  /** 操作参数 */
  params: {
    // 目标定位
    day_index?: number
    activity_index?: number

    // 新增项数据
    attraction?: {
      name: string
      location?: string
      duration?: string
      preferred_time?: string
      description?: string
    }

    // 重排序参数
    from_day?: number
    from_index?: number
    to_day?: number
    to_index?: number

    // 时间修改
    new_time?: string

    // 重生成选项
    regeneration_hints?: {
      keep_attractions?: string[]    // 保留的景点
      exclude_attractions?: string[] // 排除的景点
      preferences?: string[]         // 新增偏好
      budget_adjustment?: number     // 预算调整
    }

    // 多天操作范围
    day_range?: {
      start_day: number
      end_day: number
    }
  }

  /** 修改原因（AI 上下文） */
  reason?: string
}

/**
 * confirm_itinerary_modification 工具参数
 */
export interface ConfirmItineraryModificationParams {
  /** 修改预览 ID */
  modification_id: string

  /** 用户调整（可选） */
  user_adjustments?: {
    // 允许在确认时微调
    time_adjustments?: Array<{
      day_index: number
      activity_index: number
      new_time: string
    }>
  }
}
```

---

## 六、文件变更清单

### 6.1 新增文件

| 文件路径 | 说明 | 预估行数 |
|---------|------|---------|
| `lib/stores/modification-store.ts` | 修改状态管理 (Zustand) | 80 |
| `lib/chat/modification-workflow.ts` | 智能重规划工作流（调用 LangGraph） | 200 |
| `lib/chat/modification-cache.ts` | Pending 修改缓存管理 | 60 |
| `components/chat/ModificationPreviewCard.tsx` | 修改预览卡片组件 | 150 |
| `components/chat/ModificationDiffView.tsx` | 前后对比视图组件 | 120 |
| `components/chat/ModificationImpactSummary.tsx` | 影响评估组件 | 60 |
| `components/chat/TripChatPanel.tsx` | 行程详情页对话侧边栏 | 200 |
| `app/api/trips/[id]/modifications/route.ts` | 修改预览 API（可选，用于 REST 调用） | 100 |

### 6.2 修改文件

| 文件路径 | 修改内容 | 预估改动 |
|---------|---------|---------|
| `lib/chat/tools.ts` | 添加 `prepare_itinerary_modification` 和 `confirm_itinerary_modification` 工具定义 | +100 行 |
| `lib/chat/executor.ts` | 实现 `prepareItineraryModification()` 和 `confirmItineraryModification()` 方法 | +250 行 |
| `lib/chat/types.ts` | 添加 `ModificationPreview`, `ModificationChange` 等类型定义 | +80 行 |
| `hooks/useChatAgent.ts` | 扩展支持修改预览状态管理 | +60 行 |
| `components/chat/MessageList.tsx` | 支持渲染 ModificationPreviewCard | +30 行 |
| `components/chat/ToolCallCard.tsx` | 添加新工具的图标和描述 | +10 行 |
| `components/chat/index.ts` | 导出新组件 | +5 行 |
| `app/dashboard/trips/[id]/page.tsx` | 添加对话侧边栏入口和状态 | +50 行 |

---

## 七、实现阶段

### Phase 1: 基础修改 + 预览确认 ✅

**目标**: 实现基础修改操作的预览确认流程

**预计工作量**: 3-4 天

**实际完成日期**: 2025-12-09

#### 1.1 类型定义 (`lib/chat/types.ts`) ✅

- [x] 添加 `ItineraryModificationOperation` 枚举
- [x] 添加 `ModificationPreview` 接口
- [x] 添加 `ModificationChange` 接口
- [x] 添加 `DayPlanSummary` 接口
- [x] 添加 `PrepareItineraryModificationParams` 接口
- [x] 添加 `ConfirmItineraryModificationParams` 接口

#### 1.2 工具定义 (`lib/chat/tools.ts`) ✅

- [x] 添加 `prepare_itinerary_modification` 工具定义
- [x] 添加 `confirm_itinerary_modification` 工具定义
- [x] 更新 `TOOL_DESCRIPTIONS` 和 `TOOL_ICONS`

#### 1.3 缓存管理 (`lib/chat/modification-cache.ts`) ✅

- [x] 实现内存缓存（Map）
- [x] 10 分钟过期机制
- [x] `set()`, `get()`, `delete()` 方法

#### 1.4 执行器实现 (`lib/chat/executor.ts`) ✅

- [x] 实现 `prepareItineraryModification()` 方法
  - 加载行程
  - 计算修改后的行程
  - 生成 before/after 对比
  - 生成变更列表
  - 计算影响评估
  - 缓存 pending 修改
  - 返回预览数据
- [x] 实现 `confirmItineraryModification()` 方法
  - 从缓存获取 pending 修改
  - 应用到数据库
  - 清除缓存
  - 返回更新结果

#### 1.5 状态管理 (`lib/stores/modification-store.ts`) ✅

- [x] 创建 Zustand store
- [x] `pendingModification` 状态
- [x] `setPendingModification()` 方法
- [x] `confirmModification()` 方法
- [x] `cancelModification()` 方法

#### 1.6 预览组件 (`components/chat/ModificationPreviewCard.tsx`) ✅

- [x] 操作类型标题和图标
- [x] 变更摘要列表
- [x] 影响评估显示
- [x] 确认/取消按钮
- [x] 处理中状态
- [x] 详情对比视图（可展开）

#### 1.7 消息列表集成 (`components/chat/MessageList.tsx`) ✅

- [x] 识别 `modification_preview` 类型工具结果
- [x] 渲染 ModificationPreviewCard

#### 1.8 Hook 扩展 (`hooks/useChatAgent.ts`) ✅

- [x] 处理 `prepare_itinerary_modification` 工具结果
- [x] 更新 modification store
- [x] 实现确认/取消回调
- [x] 处理确认后的行程刷新

### Phase 2: 前后对比 + 详情编辑 ✅

**目标**: 增强预览体验，支持查看详细对比和微调

**预计工作量**: 2-3 天

**实际完成日期**: 2025-12-09

#### 2.1 对比视图组件 (`components/chat/ModificationDiffView.tsx`) ✅

- [x] 左右对比布局
- [x] 变更项高亮
- [x] 时间线对比
- [x] 成本对比

#### 2.2 影响评估组件 (`components/chat/ModificationImpactSummary.tsx`) ✅

- [x] 成本变化显示
- [x] 时间影响显示
- [x] 警告信息显示
- [x] 受影响天数标记

#### 2.3 预览微调功能 ✅

- [x] 在预览中修改时间
- [ ] 在预览中调整顺序（Phase 3 实现）
- [ ] 在预览中编辑新增项详情（Phase 3 实现）
- [x] 更新 `confirm_itinerary_modification` 支持 `user_adjustments`

### Phase 3: 智能重规划 ✅

**目标**: 实现 AI 驱动的智能优化功能

**预计工作量**: 4-5 天

**实际完成日期**: 2025-12-10

#### 3.1 重规划工作流 (`lib/chat/modification-workflow.ts`) ✅

- [x] 实现 `optimizeRoute()` - 路线优化算法
  - 基于地理位置的最近邻贪心算法
  - 考虑交通时间和距离
  - 自动重新分配时间
- [x] 实现 `replanDay()` - 调用 AI 重新规划
  - 复用 DeepSeek API
  - 支持保留/排除景点约束
  - 支持偏好和预算调整
- [x] 实现 `adjustForWeather()` - 天气适应
  - 获取高德天气预报
  - 自动识别户外景点
  - AI 推荐室内替代景点

#### 3.2 执行器集成 (`lib/chat/executor.ts`) ✅

- [x] 集成 `optimize_route` 操作
- [x] 集成 `replan_day` 操作
- [x] 集成 `adjust_for_weather` 操作
- [x] 错误处理和友好提示

#### 3.3 模块导出 (`lib/chat/index.ts`) ✅

- [x] 导出工作流函数
- [x] 导出类型定义

### Phase 4: 行程详情页集成 ⏳

**目标**: 在行程详情页添加对话入口

**预计工作量**: 2-3 天

#### 4.1 对话侧边栏 (`components/chat/TripChatPanel.tsx`)

- [ ] 滑出式面板组件
- [ ] 关联当前行程上下文
- [ ] 消息列表
- [ ] 输入框

#### 4.2 行程详情页修改 (`app/dashboard/trips/[id]/page.tsx`)

- [ ] 添加"AI 助手"悬浮按钮
- [ ] 集成 TripChatPanel
- [ ] 状态管理（面板开关）
- [ ] 修改后自动刷新行程显示

#### 4.3 实时同步

- [ ] 监听修改确认事件
- [ ] 刷新行程数据
- [ ] 滚动到修改的天数
- [ ] 高亮新修改的内容

---

## 八、关键代码设计

### 8.1 工具定义

```typescript
// lib/chat/tools.ts

// prepare_itinerary_modification 工具
{
  type: 'function',
  function: {
    name: 'prepare_itinerary_modification',
    description: `准备行程修改预览。分析修改影响，生成前后对比，等待用户确认后才保存。

支持的操作类型：
- 基础修改：add_attraction, remove_attraction, reorder, change_time, change_hotel, change_restaurant
- 结构变更：add_day, remove_day, split_day, merge_days
- 智能重规划：optimize_route, replan_day, adjust_for_weather
- 重生成：regenerate_day, regenerate_trip_segment

使用场景：
- 用户说"删除第二天的西湖" → operation: remove_attraction
- 用户说"把灵隐寺移到上午" → operation: change_time
- 用户说"帮我优化一下路线" → operation: optimize_route
- 用户说"博物馆闭馆了重新规划" → operation: replan_day

注意：此工具只生成预览，不会立即保存。用户需要确认后才会执行修改。`,
    parameters: {
      type: 'object',
      properties: {
        trip_id: {
          type: 'string',
          description: '要修改的行程 ID',
        },
        operation: {
          type: 'string',
          enum: [
            'add_attraction', 'remove_attraction', 'reorder', 'change_time',
            'change_hotel', 'change_restaurant',
            'add_day', 'remove_day', 'split_day', 'merge_days',
            'optimize_route', 'replan_day', 'adjust_for_weather',
            'regenerate_day', 'regenerate_trip_segment',
          ],
          description: '操作类型',
        },
        params: {
          type: 'object',
          description: '操作参数',
          properties: {
            day_index: { type: 'number', description: '天数索引（从 0 开始）' },
            activity_index: { type: 'number', description: '活动索引' },
            attraction: {
              type: 'object',
              description: '新增景点信息',
              properties: {
                name: { type: 'string' },
                location: { type: 'string' },
                duration: { type: 'string' },
                preferred_time: { type: 'string' },
              },
            },
            new_time: { type: 'string', description: '新时间，如 "09:00"' },
            from_day: { type: 'number' },
            from_index: { type: 'number' },
            to_day: { type: 'number' },
            to_index: { type: 'number' },
            regeneration_hints: {
              type: 'object',
              properties: {
                keep_attractions: { type: 'array', items: { type: 'string' } },
                exclude_attractions: { type: 'array', items: { type: 'string' } },
                preferences: { type: 'array', items: { type: 'string' } },
              },
            },
            day_range: {
              type: 'object',
              properties: {
                start_day: { type: 'number' },
                end_day: { type: 'number' },
              },
            },
          },
        },
        reason: {
          type: 'string',
          description: '用户说明的修改原因（用于 AI 上下文理解）',
        },
      },
      required: ['trip_id', 'operation', 'params'],
    },
  },
},

// confirm_itinerary_modification 工具
{
  type: 'function',
  function: {
    name: 'confirm_itinerary_modification',
    description: `确认并应用行程修改。

仅在以下情况调用：
1. 用户明确确认要执行修改
2. 用户点击了"确认修改"按钮

此工具会将 prepare_itinerary_modification 生成的预览修改保存到数据库。`,
    parameters: {
      type: 'object',
      properties: {
        modification_id: {
          type: 'string',
          description: '修改预览 ID（由 prepare_itinerary_modification 返回）',
        },
        user_adjustments: {
          type: 'object',
          description: '用户在确认前的微调（可选）',
          properties: {
            time_adjustments: {
              type: 'array',
              items: {
                type: 'object',
                properties: {
                  day_index: { type: 'number' },
                  activity_index: { type: 'number' },
                  new_time: { type: 'string' },
                },
              },
            },
          },
        },
      },
      required: ['modification_id'],
    },
  },
},
```

### 8.2 执行器核心方法

```typescript
// lib/chat/executor.ts

/**
 * 准备行程修改预览
 */
private async prepareItineraryModification(
  params: PrepareItineraryModificationParams
): Promise<ModificationPreview | { success: false; message: string }> {
  const { trip_id, operation, params: opParams, reason } = params

  // 1. 加载当前行程
  const { data: trip, error } = await this.supabase
    .from('trips')
    .select('*')
    .eq('id', trip_id)
    .eq('user_id', this.userId)
    .single()

  if (error || !trip) {
    return { success: false, message: '行程不存在或无权访问' }
  }

  // 2. 深拷贝行程作为工作副本
  const beforeItinerary = JSON.parse(JSON.stringify(trip.itinerary))
  const afterItinerary = JSON.parse(JSON.stringify(trip.itinerary))

  // 3. 根据操作类型计算修改
  const changes: ModificationChange[] = []
  let modified = false

  switch (operation) {
    case 'remove_attraction':
      // 实现删除逻辑，记录 change
      break
    case 'add_attraction':
      // 实现添加逻辑
      break
    case 'reorder':
      // 实现重排序逻辑
      break
    case 'optimize_route':
      // 调用路线优化算法
      break
    case 'replan_day':
      // 调用 AI 重规划
      break
    // ... 其他操作
  }

  // 4. 生成预览摘要
  const beforeSummary = this.generateDaySummary(beforeItinerary, changes)
  const afterSummary = this.generateDaySummary(afterItinerary, changes)

  // 5. 计算影响评估
  const impact = this.calculateImpact(beforeItinerary, afterItinerary, changes)

  // 6. 创建预览对象
  const preview: ModificationPreview = {
    id: `mod_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    tripId: trip_id,
    operation,
    before: {
      days: beforeSummary,
      totalCost: this.calculateTotalCost(beforeItinerary),
    },
    after: {
      days: afterSummary,
      totalCost: this.calculateTotalCost(afterItinerary),
    },
    changes,
    impact,
    createdAt: Date.now(),
    expiresAt: Date.now() + 10 * 60 * 1000, // 10 分钟后过期
    status: 'pending',
  }

  // 7. 缓存 pending 修改（包含完整的 afterItinerary）
  modificationCache.set(preview.id, {
    preview,
    afterItinerary,
  })

  return preview
}

/**
 * 确认并应用修改
 */
private async confirmItineraryModification(
  params: ConfirmItineraryModificationParams
): Promise<{ success: boolean; message: string; itinerary?: Itinerary }> {
  const { modification_id, user_adjustments } = params

  // 1. 从缓存获取 pending 修改
  const cached = modificationCache.get(modification_id)
  if (!cached) {
    return { success: false, message: '修改预览已过期或不存在，请重新发起修改' }
  }

  let { afterItinerary } = cached

  // 2. 应用用户微调（如果有）
  if (user_adjustments?.time_adjustments) {
    for (const adj of user_adjustments.time_adjustments) {
      afterItinerary.days[adj.day_index].activities[adj.activity_index].time = adj.new_time
    }
  }

  // 3. 保存到数据库
  const { error } = await this.supabase
    .from('trips')
    .update({
      itinerary: afterItinerary,
      updated_at: new Date().toISOString(),
    })
    .eq('id', cached.preview.tripId)

  if (error) {
    return { success: false, message: '保存失败: ' + error.message }
  }

  // 4. 清除缓存
  modificationCache.delete(modification_id)

  return {
    success: true,
    message: '行程修改已保存',
    itinerary: afterItinerary,
  }
}
```

### 8.3 状态管理

```typescript
// lib/stores/modification-store.ts

import { create } from 'zustand'
import type { ModificationPreview } from '@/lib/chat/types'

interface ModificationState {
  /** 当前待确认的修改预览 */
  pendingModification: ModificationPreview | null

  /** 是否正在处理 */
  isProcessing: boolean

  /** 设置待确认修改 */
  setPendingModification: (preview: ModificationPreview | null) => void

  /** 确认修改 */
  confirmModification: (modificationId: string, userAdjustments?: any) => Promise<boolean>

  /** 取消修改 */
  cancelModification: () => void

  /** 设置处理状态 */
  setProcessing: (processing: boolean) => void
}

export const useModificationStore = create<ModificationState>((set, get) => ({
  pendingModification: null,
  isProcessing: false,

  setPendingModification: (preview) => set({ pendingModification: preview }),

  confirmModification: async (modificationId, userAdjustments) => {
    set({ isProcessing: true })
    try {
      // 这里实际会通过对话发送确认工具调用
      // 或者直接调用 API
      return true
    } finally {
      set({ isProcessing: false, pendingModification: null })
    }
  },

  cancelModification: () => set({ pendingModification: null }),

  setProcessing: (processing) => set({ isProcessing: processing }),
}))
```

### 8.4 预览组件

```tsx
// components/chat/ModificationPreviewCard.tsx

import type { ModificationPreview } from '@/lib/chat/types'

const OPERATION_LABELS: Record<string, string> = {
  add_attraction: '添加景点',
  remove_attraction: '删除景点',
  reorder: '调整顺序',
  change_time: '修改时间',
  optimize_route: '优化路线',
  replan_day: '重新规划',
  // ...
}

const OPERATION_ICONS: Record<string, string> = {
  add_attraction: '➕',
  remove_attraction: '❌',
  reorder: '🔄',
  change_time: '⏰',
  optimize_route: '🗺️',
  replan_day: '✨',
  // ...
}

interface ModificationPreviewCardProps {
  preview: ModificationPreview
  onConfirm: () => void
  onCancel: () => void
  onViewDetails: () => void
  isProcessing?: boolean
}

export function ModificationPreviewCard({
  preview,
  onConfirm,
  onCancel,
  onViewDetails,
  isProcessing,
}: ModificationPreviewCardProps) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      {/* 头部 */}
      <div className="px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          <span className="text-xl">{OPERATION_ICONS[preview.operation] || '✏️'}</span>
          <h3 className="font-medium text-gray-900 dark:text-white">行程修改预览</h3>
          <span className="px-2 py-0.5 text-xs bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-100 rounded">
            {OPERATION_LABELS[preview.operation] || preview.operation}
          </span>
        </div>
      </div>

      {/* 变更摘要 */}
      <div className="p-4">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">📍 变更摘要</h4>
        <ul className="space-y-1">
          {preview.changes.map((change, idx) => (
            <li key={idx} className="flex items-center gap-2 text-sm">
              <span>{change.type === 'add' ? '➕' : change.type === 'remove' ? '❌' : '📝'}</span>
              <span className="text-gray-600 dark:text-gray-400">{change.description}</span>
            </li>
          ))}
        </ul>
      </div>

      {/* 影响评估 */}
      <div className="px-4 pb-4">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">📊 影响评估</h4>
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div>
            <span className="text-gray-500">受影响天数：</span>
            <span>第 {preview.impact.affectedDays.map(d => d + 1).join(', ')} 天</span>
          </div>
          <div>
            <span className="text-gray-500">预算变化：</span>
            <span className={preview.impact.costDelta > 0 ? 'text-red-500' : 'text-green-500'}>
              {preview.impact.costDelta > 0 ? '+' : ''}¥{preview.impact.costDelta}
            </span>
          </div>
        </div>

        {/* 警告信息 */}
        {preview.impact.warnings.length > 0 && (
          <div className="mt-2 text-sm text-yellow-600 dark:text-yellow-400">
            {preview.impact.warnings.map((w, i) => (
              <p key={i}>⚠️ {w}</p>
            ))}
          </div>
        )}
      </div>

      {/* 操作按钮 */}
      <div className="px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
        <button
          onClick={onCancel}
          disabled={isProcessing}
          className="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded"
        >
          取消
        </button>
        <button
          onClick={onViewDetails}
          disabled={isProcessing}
          className="px-3 py-1.5 text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/50 rounded"
        >
          查看详情
        </button>
        <button
          onClick={onConfirm}
          disabled={isProcessing}
          className="px-3 py-1.5 text-sm bg-blue-600 text-white hover:bg-blue-700 rounded disabled:opacity-50"
        >
          {isProcessing ? '处理中...' : '确认修改'}
        </button>
      </div>
    </div>
  )
}
```

---

## 九、验收标准

### 9.1 功能验收

**Phase 1 验收**:
- [ ] 用户可以通过对话描述修改需求（删除景点、添加景点等）
- [ ] AI 正确识别修改意图并调用 `prepare_itinerary_modification`
- [ ] 显示修改预览卡片，包含变更摘要和影响评估
- [ ] 用户点击"确认"后修改保存到数据库
- [ ] 用户点击"取消"后修改被丢弃
- [ ] 预览 10 分钟后自动过期

**Phase 2 验收**:
- [ ] 点击"查看详情"显示前后对比视图
- [ ] 对比视图正确高亮变更项
- [ ] 可以在预览中微调时间

**Phase 3 验收**:
- [ ] `optimize_route` 操作正确优化路线顺序
- [ ] `replan_day` 操作调用 AI 重新规划
- [ ] 重规划过程显示进度

**Phase 4 验收**:
- [ ] 行程详情页有"AI 助手"入口
- [ ] 对话侧边栏正常显示
- [ ] 修改确认后行程详情页自动刷新
- [ ] 刷新后滚动到修改的天数

### 9.2 边界情况测试

- [ ] 并发修改：同时修改同一行程的处理
- [ ] 过期处理：预览过期后的友好提示
- [ ] 网络错误：保存失败后的恢复机制
- [ ] 数据冲突：行程被其他地方修改后的处理
- [ ] 权限验证：确保只能修改自己的行程

---

## 十、风险与缓解

| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| 预览计算耗时 | 中 | 基础操作本地计算，复杂操作显示加载状态 |
| 缓存过期 | 低 | 清晰的过期提示，引导用户重新发起 |
| AI 重规划质量 | 中 | 复用已验证的 LangGraph Agent |
| 状态同步 | 中 | 确认后立即刷新，使用乐观更新 |
| 移动端体验 | 中 | 侧边栏改为底部弹出面板 |

---

## 更新日志

| 日期 | 版本 | 更新内容 |
|------|------|---------|
| 2025-12-08 | v1.0 | 初始计划文档 |
| 2025-12-09 | v1.1 | Phase 1 已完成：实现基础修改 + 预览确认流程 |
| 2025-12-09 | v1.2 | Phase 2 已完成：实现前后对比视图、影响评估组件、时间微调功能 |
| 2025-12-10 | v1.3 | Phase 3 已完成：实现智能重规划（路线优化、AI 重规划、天气适应） |
