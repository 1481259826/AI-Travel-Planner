...
> ✅ **已完成** (2024-12-12): 所有代码已使用新的类式 API，兼容层文件已删除。
...
> ✅ **已完成** (2024-12-12): 所有导入已更新，`db` CRUD 操作已迁移到 `lib/database/crud.ts`，兼容层文件已删除。
...
> ✅ **已完成** (2024-12-12): 该文件无任何外部引用，已直接删除。
...
> ✅ **已完成** (2024-12-12): 采用选项 B，已删除未使用的服务文件，保留 `geocoding.service.ts`。

### 3.1 评估结果

**关键发现**：
- `lib/repositories/` 目录不存在，意味着依赖 Repository 模式的服务（`trip.service.ts`, `expense.service.ts`）根本无法正常工作
- `base.service.ts` 依赖不存在的 `IRepository` 接口
- `ai.service.ts` 虽然可独立工作，但无任何代码引用
- `geocoding.service.ts` 独立实现（不依赖 BaseService），被 `coordinate-fixer.ts` 实际使用

### 3.2 执行结果

| 服务 | 行数 | 处理方式 |
|------|------|---------|
| `trip.service.ts` | 346 | ❌ 已删除（依赖不存在的 Repository） |
| `expense.service.ts` | 292 | ❌ 已删除（依赖不存在的 Repository） |
| `ai.service.ts` | 256 | ❌ 已删除（无任何引用） |
| `base.service.ts` | 189 | ❌ 已删除（Repository 模式未实现） |
| `geocoding.service.ts` | 392 | ✅ 保留（被 `coordinate-fixer.ts` 使用） |

### 3.3 最终状态

`lib/services/` 目录现仅包含：
- `geocoding.service.ts` - 提供地理编码、POI 搜索、坐标转换功能
...
| 阶段 | 任务 | 状态 | 完成日期 |
|------|------|------|---------|
| 1 | `lib/api-keys.ts` 迁移 | ✅ 已完成 | 2024-12-12 |
| 2 | `lib/supabase.ts` 迁移 | ✅ 已完成 | 2024-12-12 |
| 3 | `lib/check-api-keys.ts` 迁移 | ✅ 已完成 | 2024-12-12 |
| 4 | 服务层评估与处理 | ✅ 已完成 | 2024-12-12 |
| 5 | `useChatAgent.ts` 拆分 | 待开始 | - |
| 6 | `sync.ts` 拆分 | 待开始 | - |
| 7 | 常量模块整合 | 待开始 | - |
| 8 | 删除向后兼容层文件 | ✅ 已完成 | 2024-12-12 |
