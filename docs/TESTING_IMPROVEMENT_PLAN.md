...
**Phase 6: E2E 测试 (可选，0/4 待完成)**:
1. 安装 Playwright
2. 创建行程 E2E 测试 (登录 → 填写表单 → 生成行程)
3. 导出 PDF E2E 测试 (查看行程 → 导出 PDF)
4. 分享行程 E2E 测试 (创建分享 → 复制链接 → 匿名访问)

**预计时间**: 5-6 小时

**或者 Phase 7: 测试基础设施 (推荐优先，0/5 待完成)**:
1. 优化现有 Mock 配置（Supabase、高德地图、AI 模型）
2. 创建测试辅助函数和数据生成器
...
**注意事项**:
- ✅ Phase 1-5 已完成，测试覆盖率从 5% 提升至 ~50%+
- 运行大量测试时可能出现内存不足，建议增加 Node.js 内存限制：
  ```bash
  NODE_OPTIONS="--max-old-space-size=4096" npm test
  ```
- 所有集成测试使用 fake-indexeddb 模拟 IndexedDB
- 建议优先完成 Phase 7（测试基础设施）以提升测试质量
