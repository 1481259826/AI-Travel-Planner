...
│   ├── useApiKeyCheck.ts         # API Key 检查 Hook
│   ├── useOfflineTrip.ts         # 单个离线行程 Hook
│   ├── useOfflineTrips.ts        # 多个离线行程 Hook
│   ├── useSync.ts                # 数据同步 Hook
│   └── useServerStatus.ts        # 服务器状态检查 Hook
├── lib/                          # 工具库
│   ├── database/                 # Supabase 模块化目录
│   │   ├── client.ts             # 客户端初始化
│   │   ├── auth.ts               # 认证操作
│   │   └── schema.ts             # 类型定义
...
│   │   └── checker.ts            # 可用性检查
│   ├── pdf/                      # PDF 导出模块
│   │   ├── sections/             # PDF 各部分（封面、总览、每日计划等）
│   │   └── utils/                # PDF 工具函数
│   ├── services/                 # 业务服务层
│   │   ├── trip.service.ts       # 行程服务
│   │   ├── expense.service.ts    # 费用服务
│   │   └── ai.service.ts         # AI 调用服务
│   ├── repositories/             # 数据仓储层
│   │   ├── trip.repository.ts    # 行程仓储
│   │   └── expense.repository.ts # 费用仓储
│   ├── stores/                   # 状态管理（Zustand）
│   ├── config/                   # 配置模块
│   ├── constants/                # 常量定义
│   ├── errors/                   # 自定义错误类
