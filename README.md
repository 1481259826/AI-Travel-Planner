# AI 旅行规划师 (AI Travel Planner)

一个基于 AI 技术的智能旅行规划 Web 应用，支持多模型选择、语音输入、智能行程生成、费用管理等功能。

## ✨ 功能特性

### 已实现功能

- **🎤 语音输入** - 基于 Web Speech API 的语音识别，支持中文语音输入
- **🤖 多 AI 模型支持** - 支持 DeepSeek Chat、DeepSeek Reasoner、Qwen2.5 72B (ModelScope)
- **🧠 LangGraph 多智能体架构** - 8 个专家 Agent 协作，智能行程规划（[查看文档](docs/架构设计/多智能体架构升级计划.md)）
- **💬 对话助手** - 自然语言交互的旅行规划助手，支持 9+ 工具调用、SSE 流式响应
- **✏️ 对话式行程修改** - 通过自然语言增删景点、更换酒店/餐厅，实时预览修改影响
- **🔄 Human-in-the-Loop** - 工作流中断与恢复机制，支持行程审核和预算决策人工干预
- **📋 智能行程规划** - AI 生成详细旅行计划，包含每日行程、景点推荐、餐厅推荐
- **📑 旅行模板** - 保存行程为模板，快速复用常用路线和偏好设置
- **📜 历史行程** - 查看历史生成的行程记录，支持恢复和对比
- **💰 费用预估** - 智能预算分析和费用估算
- **🔐 用户认证** - Supabase 认证集成，支持邮箱/密码注册登录
- **📊 行程管理** - 创建、保存和查看多个旅行计划
- **🗺️ 地图集成** - 高德地图显示景点位置、路线规划（[查看文档](docs/MAP_INTEGRATION.md)）
- **📲 高德 APP 联动** - 一键同步行程到高德地图 APP，生成专属导航地图
- **💵 费用追踪** - 语音录入开销、实时预算对比、费用分类管理
- **📈 数据可视化** - 预算使用图表、费用分布分析、每日开销趋势（[查看文档](docs/BUDGET_VISUALIZATION.md)）
- **🔗 行程分享** - 生成分享链接、二维码、支持社交媒体分享（[查看文档](docs/SHARE_FEATURE.md)）
- **📱 PWA 支持** - 安装到主屏幕，像原生应用一样使用
- **💾 离线缓存** - 查看和编辑已缓存的行程，支持离线访问（[查看文档](docs/OFFLINE_USAGE.md)）
- **⚙️ 用户设置** - 账户管理、密码修改、主题切换、API Keys 管理（[查看文档](docs/SETTINGS_GUIDE.md)）
- **📄 PDF 导出** - 导出行程为 PDF 文档，支持打印优化版本，完美支持中文
- **🔧 进程管理** - 自动检测和清理僵尸进程，支持 PM2 生产级管理（[查看文档](docs/PROCESS_MANAGEMENT.md)）

### 计划功能

- 百度地图支持
- 多币种支持
- 费用预测分析
- 分享访问统计
- 离线创建新行程
- 推送通知

## 🚀 快速开始

> 💡 **推荐启动方式**: 使用 `npm run dev:safe` 安全启动，自动避免僵尸进程问题。详见 [快速启动指南](docs/QUICK_START.md)。

### 1. 安装依赖

```bash
npm install
```

### 2. 配置环境变量

复制 `.env.example` 为 `.env.local`：

```bash
cp .env.example .env.local
```

然后填入以下信息：

```env
# Supabase（必需）
NEXT_PUBLIC_SUPABASE_URL=your_supabase_project_url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_supabase_anon_key
SUPABASE_SERVICE_ROLE_KEY=your_supabase_service_role_key  # 分享功能必需

# DeepSeek API（必需 - AI 行程生成）
DEEPSEEK_API_KEY=your_deepseek_api_key
DEEPSEEK_BASE_URL=https://api.deepseek.com

# ModelScope API（可选 - Qwen 模型）
MODELSCOPE_API_KEY=your_modelscope_api_key
MODELSCOPE_BASE_URL=https://api-inference.modelscope.cn/v1/

# 高德地图 API（必需 - 用于行程地图显示）
NEXT_PUBLIC_MAP_API_KEY=your_amap_api_key
AMAP_WEB_SERVICE_KEY=your_amap_web_service_key  # Web服务API Key（后端使用）

# 高德官方 MCP 服务配置（可选 - 推荐使用）
AMAP_MCP_MODE=streamable-http                   # 连接模式：sse | streamable-http（推荐）
AMAP_MCP_SSE_URL=https://mcp.amap.com/sse       # SSE 服务地址（使用默认值即可）
AMAP_MCP_HTTP_URL=https://mcp.amap.com/mcp      # Streamable HTTP 地址（使用默认值即可）

# 数据加密密钥（必需 - 用于 API Key 加密存储）
ENCRYPTION_KEY=your_32_char_or_longer_encryption_key_here

# 应用配置
NEXT_PUBLIC_BASE_URL=http://localhost:3008  # 开发环境；生产环境改为实际域名

# LangGraph 多智能体配置（可选）
NEXT_PUBLIC_USE_LANGGRAPH=true              # 启用 LangGraph 架构
LANGGRAPH_TRACER=console                    # 追踪后端：console | json | langsmith
LANGGRAPH_METRICS_ENABLED=true              # 启用指标收集

# 对话助手与 HITL 配置（可选）
NEXT_PUBLIC_USE_CHAT_AGENT=true             # 启用对话助手（默认开启）
NEXT_PUBLIC_USE_HITL=false                  # 启用 Human-in-the-Loop（默认关闭）

# 其他可选服务
VOICE_API_KEY=your_voice_api_key
```

#### 获取 API Key

**Supabase:**
1. 访问 [supabase.com](https://supabase.com) 创建项目
2. 进入 Settings → API
3. 复制以下密钥：
   - `Project URL` → `NEXT_PUBLIC_SUPABASE_URL`
   - `anon public` → `NEXT_PUBLIC_SUPABASE_ANON_KEY`
   - `service_role` → `SUPABASE_SERVICE_ROLE_KEY` ⚠️ 保密！仅服务端使用

**DeepSeek (必需):**
1. 访问 [platform.deepseek.com](https://platform.deepseek.com)
2. 创建 API Key
3. DeepSeek 提供优秀的中文支持和高性价比

**ModelScope (可选):**
1. 访问 [modelscope.cn](https://modelscope.cn)
2. 进入体验平台 → API 推理服务
3. 创建并获取 API Key（支持 Qwen 系列模型）

**高德地图 (必需):**
1. 访问 [lbs.amap.com](https://lbs.amap.com)
2. 创建应用并获取 Web 端（JS API）Key 和 Web 服务 API Key
3. Web 服务 Key 需要开通以下服务权限：
   - 地理编码/逆地理编码
   - 搜索服务 POI
   - 路径规划
   - 天气查询
4. 详细说明查看 [docs/MAP_INTEGRATION.md](docs/MAP_INTEGRATION.md)

> 💡 **高德官方 MCP 服务**：项目已集成高德官方 MCP 服务，自动使用 Web 服务 Key 连接。支持 12+ 官方工具，包括生成专属地图、导航唤端等高级功能。详见 [docs/架构设计/高德官方MCP服务升级计划.md](docs/架构设计/高德官方MCP服务升级计划.md)

**加密密钥 (必需):**
使用 Node.js 生成安全的随机密钥：
```bash
node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
```
⚠️ 密钥至少 32 个字符，不要将密钥提交到版本控制

### 3. 设置数据库

在 Supabase SQL Editor 中运行统一的数据库初始化脚本：

```bash
# 复制 database/init.sql 的全部内容到 Supabase SQL Editor 并执行
# 一次性完成所有数据库设置，包含：
# - 4 个核心表：profiles、trips、expenses、api_keys
# - 完整的 RLS 安全策略
# - 性能优化索引
# - 自动更新触发器
# - 用户设置功能（主题、默认偏好、API Keys 加密存储）
```

**已有项目？** 无需担心！脚本包含向后兼容逻辑，会自动检测并添加缺失字段，安全升级数据库到最新版本。

### 4. 启动项目

**推荐方式（自动处理僵尸进程）：**
```bash
npm run dev:safe
```

**标准方式：**
```bash
npm run dev
```

**生产级管理（需要 PM2）：**
```bash
npm install -g pm2
npm run dev:pm2
```

访问 http://localhost:3008

**运行测试：**
```bash
npm test                  # 运行单元测试
npm run test:coverage     # 测试覆盖率报告
npm run test:e2e          # Playwright E2E 测试
```

**注意**：
- 默认端口为 3008，可在 `package.json` 中修改
- 更多启动选项和进程管理，请查看 [进程管理指南](docs/PROCESS_MANAGEMENT.md)

## 📖 使用指南

### 创建行程

1. **注册/登录** - 首次使用需要注册账号
2. **创建新行程** - 点击"创建新行程"按钮
3. **填写信息**：
   - **出发地**（支持语音输入）- 用于准确估算交通费用
   - **目的地**（支持语音输入）
   - 出发和返回日期
   - 预算金额
   - 出行人数
   - 旅行偏好
   - 选择 AI 模型
4. **生成行程** - 点击"生成旅行计划"，AI 将在几秒内生成详细行程
5. **查看详情** - 自动跳转到行程详情页，查看完整计划和地图

### 用户设置

1. **进入设置** - 点击 Dashboard 右上角的「设置」按钮
2. **账户信息** - 修改用户名、头像
3. **安全设置** - 修改密码（带强度验证）
4. **偏好设置**：
   - 选择主题（浅色/深色/跟随系统）
   - 设置默认 AI 模型
   - 设置默认预算和出发地
5. **API Keys 管理**：
   - 添加您自己的 DeepSeek/ModelScope/高德地图 API Keys
   - 系统将优先使用您的 Keys（AES-256 加密存储）
   - 测试 Key 有效性，管理多个备用 Keys

详细说明请查看 [docs/SETTINGS_GUIDE.md](docs/SETTINGS_GUIDE.md)

### 对话助手

对话助手提供自然语言交互的旅行规划体验：

1. **进入对话** - 点击 Dashboard 导航栏的「对话助手」或行程详情页的对话入口
2. **创建会话** - 点击侧边栏的「新建会话」创建对话
3. **对话交互**：
   - 直接用自然语言描述需求（如"帮我规划一个杭州3天游"）
   - 支持关联已有行程进行修改或查询
   - 实时显示工具调用过程和结果
4. **行程生成**：
   - 在对话中直接创建行程（自动弹出表单确认）
   - 实时显示多智能体生成进度
   - 生成完成后可直接跳转查看
5. **行程修改**：
   - 增删景点（"在第二天添加西湖"、"删除灵隐寺"）
   - 更换酒店（"把酒店换成万豪"）
   - 更换餐厅（"第一天午餐改成外婆家"）
   - 实时预览修改影响（成本变化、时间调整）
   - 支持确认或取消修改
6. **历史行程**：
   - 侧边栏查看历史生成记录
   - 恢复或对比历史版本

> 💡 通过 `NEXT_PUBLIC_USE_CHAT_AGENT=true` 环境变量控制（默认开启）

### 旅行模板

保存常用行程为模板，快速复用：

1. **保存模板** - 在行程详情页点击「保存为模板」
2. **管理模板** - 在创建行程页面查看和管理已保存的模板
3. **使用模板** - 选择模板快速填充行程信息
4. **编辑模板** - 修改模板名称、描述和默认设置

### 分享行程

1. **打开行程详情** - 进入想要分享的行程页面
2. **点击分享按钮** - 点击顶部的"分享行程"按钮
3. **开启公开分享** - 打开"公开分享"开关
4. **获取分享链接** - 系统自动生成唯一的分享链接
5. **分享给他人**：
   - 复制链接直接分享
   - 显示二维码供扫描
   - 分享到社交媒体（微信、微博、Twitter）
6. **管理分享** - 随时可以关闭公开分享或重新开启

详细说明请查看 [docs/SHARE_FEATURE.md](docs/SHARE_FEATURE.md)

### 同步到高德地图

1. **打开行程详情** - 进入想要同步的行程页面
2. **点击同步按钮** - 点击顶部的"同步到高德"按钮
3. **确认同步** - 查看待同步的景点列表，确认同步
4. **使用方式**：
   - **手机扫码** - 打开高德地图 APP 扫描二维码
   - **复制链接** - 在手机浏览器打开链接，自动跳转高德 APP
5. **在高德中使用**：
   - 查看所有景点标记的专属地图
   - 一键导航到任意景点
   - 规划最优游览路线

> 💡 **提示**：需要配置高德 Web 服务 API Key 才能使用此功能

### AI 模型选择

本项目支持多个 AI 模型，可根据需求选择：

- **DeepSeek Chat** - 中文支持优秀，性价比高，推荐日常使用
- **DeepSeek Reasoner** - 深度推理模型，适合复杂行程规划
- **Qwen2.5 72B (ModelScope)** - 阿里巴巴开源大模型，中文能力强

详细说明请查看 [docs/MODEL_SELECTION.md](docs/MODEL_SELECTION.md)

## 🛠️ 技术栈

### 前端
- **框架**: Next.js 15 (App Router)
- **语言**: TypeScript
- **样式**: Tailwind CSS
- **图标**: Lucide React
- **状态管理**: Zustand
- **PWA**: next-pwa + Workbox
- **离线存储**: IndexedDB (idb)
- **PDF 生成**: jsPDF + jspdf-autotable + html2canvas
- **图表**: Recharts

### 后端
- **认证与数据库**: Supabase (PostgreSQL + RLS)
- **API**: Next.js API Routes

### AI 集成
- **大语言模型**: DeepSeek + ModelScope (Qwen)
- **多智能体编排**: LangGraph (状态图编排 + 条件分支 + 循环重试)
- **MCP 协议**: 高德官方 MCP 服务（SSE/Streamable HTTP）
- **语音识别**: Web Speech API
- **图片服务**: 高德地图 POI 照片

### 测试
- **单元测试**: Vitest + Testing Library
- **E2E 测试**: Playwright
- **测试覆盖率**: 80%+ 阈值
- **Mock**: MSW (Mock Service Worker)

### 开发工具
- **进程管理**: PM2（生产级进程管理）
- **健康监控**: 自定义健康检查脚本
- **加密**: crypto-js（AES-256 加密）
- **日期处理**: date-fns
- **数据验证**: Zod
- **CI/CD**: GitHub Actions

## 🧠 LangGraph 多智能体架构

项目采用 LangGraph 状态图编排多个专家 Agent 协作完成行程规划，通过 Feature Flag 控制启用：

```bash
# 在 .env.local 中启用
NEXT_PUBLIC_USE_LANGGRAPH=true
```

### 架构概览

```
用户输入
    │
    ▼
┌───────────────────┐
│  Weather Scout    │  获取天气 + 生成策略标签
└───────────────────┘
    │
    ▼
┌───────────────────┐
│ Itinerary Planner │  生成行程骨架
└───────────────────┘
    │ [HITL: 行程审核中断点]
    ▼
┌───────────────────┐
│Attraction Enricher│  增强景点详情
└───────────────────┘
    │
    ├──────────────────┬──────────────────┐
    ▼                  ▼                  ▼
┌──────────┐    ┌──────────┐    ┌──────────┐
│  Hotel   │    │Transport │    │  Dining  │    并行执行
│  Agent   │    │  Agent   │    │  Agent   │
└──────────┘    └──────────┘    └──────────┘
    │                  │                  │
    └──────────────────┴──────────────────┘
                       │
                       ▼
              ┌───────────────────┐
              │  Budget Critic    │  预算审计（超支自动重试）
              └───────────────────┘
                       │ [HITL: 预算决策中断点]
                       ▼
              ┌───────────────────┐
              │  Finalize Agent   │  生成最终行程
              └───────────────────┘
```

### 专家 Agent (8 个)

| Agent | 职责 | MCP 工具 |
|-------|------|----------|
| Weather Scout | 获取天气预报，输出策略标签 | `maps_weather` |
| Itinerary Planner | 生成行程骨架（景点顺序） | `maps_search_text`, `maps_geo` |
| Attraction Enricher | 增强景点详情（门票、开放时间等） | `maps_search_text`, `maps_search_detail` |
| Accommodation | 推荐酒店住宿 | `maps_search_around` |
| Transport | 计算交通路线和费用 | `maps_driving`, `maps_transit_integrated` |
| Dining | 推荐餐厅 | `maps_search_text` |
| Budget Critic | 预算审计，超支触发重试 | 无（纯计算） |
| Finalize | 整合数据，输出最终行程 | 无 |

### 可观测性

- **追踪**: 支持 Console、JSON、LangSmith 三种追踪后端
- **指标**: Prometheus 格式指标端点 (`/api/metrics`)
- **调试页面**: 开发环境可访问 `/dashboard/debug` 查看工作流执行状态

### MCP 服务

项目已集成高德官方 MCP 服务，支持 12+ 官方工具：

| 工具名称 | 功能描述 |
|---------|---------|
| `maps_geo` | 地理编码（地址转坐标） |
| `maps_regeo` | 逆地理编码（坐标转地址） |
| `maps_weather` | 天气查询 |
| `maps_search_text` | 关键词搜索 POI |
| `maps_search_around` | 周边搜索 POI |
| `maps_search_detail` | POI 详情搜索 |
| `maps_driving` | 驾车路径规划 |
| `maps_walking` | 步行路径规划 |
| `maps_bicycling` | 骑行路径规划 |
| `maps_transit_integrated` | 公交路径规划 |
| `maps_distance` | 距离测量 |
| `amap_maps_bindmap` | 生成专属地图（高德 APP 联动） |

详细架构设计请查看 [docs/架构设计/多智能体架构升级计划.md](docs/架构设计/多智能体架构升级计划.md)

## 💬 对话助手架构

对话助手基于 Tool-Calling 模式实现，支持自然语言交互完成行程规划任务。

### 核心模块

```
lib/chat/
├── types.ts              # 类型定义（消息、会话、工具）
├── tools.ts              # 对话工具定义
├── executor.ts           # 工具执行器
├── agent.ts              # TravelChatAgent 核心类
├── modification-workflow.ts  # 行程修改工作流
├── modification-cache.ts # 修改缓存管理
└── index.ts              # 统一导出

lib/stores/
├── modification-store.ts # 修改状态管理（Zustand）
└── itinerary-store.ts    # 行程状态管理
```

### 支持的工具

| 工具名称 | 功能描述 |
|---------|---------|
| `create_trip` | 创建新行程 |
| `modify_trip` | 修改现有行程 |
| `get_trip_details` | 获取行程详情 |
| `delete_trip` | 删除行程 |
| `recommend_attractions` | 推荐景点 |
| `recommend_restaurants` | 推荐餐厅 |
| `get_weather` | 查询天气 |
| `add_expense` | 添加费用记录 |
| `get_expenses` | 获取费用列表 |

### 行程修改操作

通过 `modify_trip` 工具支持以下精细化修改操作：

| 操作类型 | 说明 | 示例 |
|---------|------|------|
| `add_attraction` | 添加景点 | "在第二天上午添加西湖" |
| `remove_attraction` | 删除景点 | "删除灵隐寺" |
| `change_hotel` | 更换酒店 | "把酒店换成杭州万豪" |
| `change_restaurant` | 更换餐厅 | "第一天午餐改成外婆家" |

修改流程：
1. AI 解析用户意图，生成修改操作
2. 系统预览修改影响（成本变化、时间调整）
3. 用户确认或取消
4. 应用修改并同步更新总价

### API 端点

- `POST /api/chat` - 发送消息（支持 SSE 流式响应）
- `POST /api/chat/generate-trip` - 对话式行程生成
- `GET/POST /api/chat/sessions` - 会话列表/创建
- `GET/PATCH/DELETE /api/chat/sessions/[id]` - 会话管理
- `GET/DELETE /api/chat/sessions/[id]/messages` - 消息历史

## 🔄 Human-in-the-Loop 工作流

HITL 机制允许工作流在关键节点暂停，等待用户审核和决策后继续执行。

### 启用方式

```bash
# 在 .env.local 中启用
NEXT_PUBLIC_USE_HITL=true
```

### 中断点

1. **行程审核中断** - Itinerary Planner 生成行程骨架后
   - 用户可查看、修改行程安排
   - 支持调整景点顺序、删除/添加景点

2. **预算决策中断** - Budget Critic 检测到预算超支时
   - 展示预算分析和超支明细
   - 用户可选择：接受当前方案、调整预算、重新规划

### 核心组件

- `InterruptModal` - 中断模态框容器
- `ItineraryReviewPanel` - 行程审核面板
- `BudgetDecisionPanel` - 预算决策面板
- `useHITLWorkflow` - HITL 工作流 Hook

### API 端点

- `POST /api/v2/workflow/start` - 启动 HITL 工作流
- `POST /api/v2/workflow/resume` - 恢复中断的工作流
- `GET /api/v2/workflow/status` - 查询工作流状态

## 📁 项目结构

```
ai-travel-planner/
├── app/                          # Next.js App Router
│   ├── api/                      # API 路由
│   │   ├── _middleware/          # API 中间件（认证、错误处理）
│   │   ├── _utils/               # API 工具函数
│   │   ├── generate-itinerary/   # 行程生成 API (v1 单体架构)
│   │   ├── v2/                   # 行程生成 API (v2 LangGraph 多智能体)
│   │   │   ├── generate-itinerary/
│   │   │   └── workflow/         # HITL 工作流 API
│   │   │       ├── start/        # 启动工作流
│   │   │       ├── resume/       # 恢复工作流
│   │   │       └── status/       # 查询状态
│   │   ├── chat/                 # 对话助手 API
│   │   │   ├── route.ts          # 发送消息（SSE 流式）
│   │   │   ├── generate-trip/    # 对话式行程生成
│   │   │   └── sessions/         # 会话管理
│   │   │       ├── route.ts      # 列表/创建
│   │   │       └── [id]/         # 单会话操作
│   │   │           ├── route.ts  # 获取/更新/删除
│   │   │           └── messages/ # 消息历史
│   │   ├── templates/            # 旅行模板 API
│   │   │   ├── route.ts          # 列表/创建模板
│   │   │   ├── [id]/             # 单模板 CRUD
│   │   │   └── from-trip/        # 从行程创建模板
│   │   ├── trip-history/         # 历史行程 API
│   │   │   ├── route.ts          # 获取历史记录列表
│   │   │   └── [id]/             # 单条历史记录操作
│   │   ├── workflow-debug/       # 工作流调试 API
│   │   ├── metrics/              # Prometheus 指标端点
│   │   ├── enrich-attraction/    # 景点信息增强 API
│   │   ├── enrich-hotel/         # 酒店信息增强 API
│   │   ├── amap-app/             # 高德 APP 联动 API（生成专属地图）
│   │   ├── expenses/             # 费用管理 API
│   │   ├── weather/              # 天气预报 API
│   │   ├── voice/                # 语音相关 API
│   │   ├── user/                 # 用户相关 API
│   │   │   ├── profile/          # 用户配置
│   │   │   ├── password/         # 密码修改
│   │   │   └── api-keys/         # API Keys 管理
│   │   └── trips/                # 行程相关 API
│   │       ├── [id]/share/       # 分享链接管理
│   │       └── share/[token]/    # 获取公开行程
│   ├── dashboard/                # 仪表板页面
│   │   ├── create/               # 创建行程
│   │   ├── settings/             # 用户设置
│   │   ├── chat/                 # 对话助手页面
│   │   ├── debug/                # 工作流调试页面（开发环境）
│   │   └── trips/[id]/           # 行程详情
│   │       └── print/            # 打印优化页面
│   ├── share/[token]/            # 公开分享页面
│   ├── login/                    # 登录页面
│   ├── register/                 # 注册页面
│   ├── providers.tsx             # 全局 Provider（主题等）
│   └── page.tsx                  # 首页
├── components/                   # React 组件
│   ├── ui/                       # UI 基础组件（button, card, dialog 等）
│   ├── shared/                   # 共享 UI 组件
│   │   ├── PhotoCarousel.tsx     # 照片轮播组件
│   │   └── RatingDisplay.tsx     # 星级评分组件
│   ├── map/                      # 地图相关组件
│   │   ├── MapLegend.tsx         # 地图图例
│   │   └── TripMapToolbar.tsx    # 地图工具栏
│   ├── chat/                     # 对话助手组件
│   │   ├── ChatInput.tsx         # 输入框组件
│   │   ├── ChatSidebar.tsx       # 会话列表侧边栏
│   │   ├── MessageList.tsx       # 消息列表展示
│   │   ├── ToolCallCard.tsx      # 工具调用卡片
│   │   ├── TripFormCard.tsx      # 行程表单卡片
│   │   ├── TripFormModal.tsx     # 行程表单模态框
│   │   ├── TripGenerationProgress.tsx  # 行程生成进度
│   │   ├── TripCompletionCard.tsx # 行程完成卡片
│   │   ├── HistoryTab.tsx        # 历史行程标签页
│   │   ├── HistoryList.tsx       # 历史行程列表
│   │   ├── HistoryItem.tsx       # 历史行程项
│   │   ├── TripChatPanel.tsx     # 行程详情页对话侧边栏
│   │   ├── ModificationPreviewCard.tsx # 行程修改预览卡片
│   │   ├── ModificationDiffView.tsx  # 修改前后对比视图
│   │   ├── ModificationImpactSummary.tsx # 修改影响摘要
│   │   └── index.ts              # 统一导出
│   ├── templates/                # 旅行模板组件
│   │   ├── TemplateCard.tsx      # 模板卡片
│   │   ├── TemplateManager.tsx   # 模板管理器
│   │   ├── TemplateEditModal.tsx # 模板编辑模态框
│   │   ├── TemplateSaveModal.tsx # 保存为模板模态框
│   │   └── index.ts              # 统一导出
│   ├── hitl/                     # Human-in-the-Loop 组件
│   │   ├── InterruptModal.tsx    # 中断模态框
│   │   ├── ItineraryReviewPanel.tsx  # 行程审核面板
│   │   ├── BudgetDecisionPanel.tsx   # 预算决策面板
│   │   └── index.ts              # 统一导出
│   ├── settings/                 # 设置页面组件
│   │   ├── ProfileForm.tsx       # 账户信息表单
│   │   ├── PasswordChangeForm.tsx # 密码修改表单
│   │   ├── PreferencesForm.tsx   # 偏好设置表单
│   │   ├── ApiKeyManager.tsx     # API Keys 管理
│   │   ├── AddApiKeyModal.tsx    # 添加 Key 模态框
│   │   └── api-keys/             # API Key 子组件
│   ├── VoiceInput.tsx            # 语音输入组件
│   ├── ModelSelector.tsx         # 模型选择器
│   ├── MapView.tsx               # 通用地图视图
│   ├── TripOverviewMap.tsx       # 行程总览地图
│   ├── AttractionCard.tsx        # 景点卡片组件
│   ├── HotelCard.tsx             # 酒店卡片组件
│   ├── ShareButton.tsx           # 分享按钮组件
│   ├── SyncToAmapButton.tsx      # 同步到高德 APP 组件
│   ├── ExpenseForm.tsx           # 费用表单
│   ├── ExpenseList.tsx           # 费用列表
│   ├── BudgetChart.tsx           # 预算图表
│   ├── ExportPdfButton.tsx       # PDF 导出按钮
│   ├── ExportPdfDialog.tsx       # PDF 导出对话框
│   ├── OfflineIndicator.tsx      # 离线状态指示器
│   ├── SyncStatus.tsx            # 同步状态组件
│   ├── InstallPrompt.tsx         # PWA 安装提示
│   └── CacheManager.tsx          # 缓存管理器
├── hooks/                        # React Hooks（18 个）
│   ├── useAuth.ts                # 认证 Hook
│   ├── useAuthFetch.ts           # 带认证的请求 Hook
│   ├── useAMapLoader.ts          # 高德地图加载 Hook
│   ├── useAmapInstance.ts        # 高德地图实例 Hook
│   ├── useAmapApp.ts             # 高德 APP 联动 Hook
│   ├── useMapMarkers.ts          # 地图标记管理 Hook
│   ├── useMapRoutes.ts           # 地图路线管理 Hook
│   ├── usePhotoCarousel.ts       # 照片轮播逻辑 Hook
│   ├── useApiKeyCheck.ts         # API Key 检查 Hook
│   ├── useLangGraphProgress.ts   # LangGraph 进度监听 Hook
│   ├── useChatAgent.ts           # 对话 Agent Hook
│   ├── useHITLWorkflow.ts        # HITL 工作流 Hook
│   ├── useOfflineTrip.ts         # 单个离线行程 Hook
│   ├── useOfflineTrips.ts        # 多个离线行程 Hook
│   ├── useSync.ts                # 数据同步 Hook
│   ├── useServerStatus.ts        # 服务器状态检查 Hook
│   ├── useTripHistory.ts         # 历史行程 Hook
│   └── useTemplates.ts           # 旅行模板 Hook
├── lib/                          # 工具库
│   ├── agents/                   # LangGraph 多智能体模块
│   │   ├── state.ts              # TripState Annotation 定义
│   │   ├── state-hitl.ts         # HITL 状态扩展
│   │   ├── workflow.ts           # StateGraph 工作流定义
│   │   ├── workflow-hitl.ts      # HITL 工作流（支持中断/恢复）
│   │   ├── mcp-client.ts         # MCP 客户端封装（本地/官方）
│   │   ├── mcp-sse-client.ts     # 高德官方 MCP SSE 客户端
│   │   ├── mcp-factory.ts        # MCP 客户端工厂（自动选择）
│   │   ├── mcp-tools.ts          # MCP 工具封装
│   │   ├── mcp-transformers.ts   # 数据格式转换器
│   │   ├── checkpointer.ts       # 检查点存储（内存/PostgreSQL）
│   │   ├── tracer.ts             # 工作流追踪器
│   │   ├── metrics.ts            # 执行指标收集
│   │   ├── cache.ts              # 缓存管理
│   │   ├── nodes/                # Agent 节点函数
│   │   │   ├── weather-scout.ts  # 天气感知 Agent
│   │   │   ├── itinerary-planner.ts # 核心规划 Agent
│   │   │   ├── accommodation.ts  # 住宿专家 Agent
│   │   │   ├── transport.ts      # 交通调度 Agent
│   │   │   ├── dining.ts         # 餐饮推荐 Agent
│   │   │   ├── budget-critic.ts  # 预算审计 Agent
│   │   │   ├── attraction-enricher.ts # 景点详情 Agent
│   │   │   └── finalize.ts       # 汇总输出 Agent
│   │   └── prompts/              # Agent System Prompts
│   ├── chat/                     # 对话助手模块
│   │   ├── types.ts              # 类型定义（消息、会话、工具）
│   │   ├── tools.ts              # 9 个对话工具定义
│   │   ├── executor.ts           # 工具执行器
│   │   ├── agent.ts              # TravelChatAgent 核心类
│   │   ├── modification-workflow.ts  # 行程修改工作流
│   │   ├── modification-cache.ts # 修改缓存管理
│   │   └── index.ts              # 统一导出
│   ├── templates/                # 旅行模板模块
│   │   ├── types.ts              # 模板类型定义
│   │   ├── service.ts            # 模板服务（CRUD）
│   │   └── index.ts              # 统一导出
│   ├── trip-history/             # 历史行程模块
│   │   ├── types.ts              # 历史记录类型定义
│   │   ├── service.ts            # 历史记录服务
│   │   └── index.ts              # 统一导出
│   ├── database/                 # Supabase 模块化目录
│   │   ├── client.ts             # 客户端初始化
│   │   ├── auth.ts               # 认证操作
│   │   └── schema.ts             # 类型定义
│   ├── api-keys/                 # API Key 管理模块
│   │   ├── client.ts             # 获取、解密、缓存
│   │   ├── validator.ts          # API Key 测试
│   │   └── checker.ts            # 可用性检查
│   ├── pdf/                      # PDF 导出模块
│   │   ├── sections/             # PDF 各部分（封面、总览、每日计划等）
│   │   └── utils/                # PDF 工具函数
│   ├── services/                 # 业务服务层
│   │   └── geocoding.service.ts  # 地理编码服务
│   ├── stores/                   # 状态管理（Zustand）
│   │   ├── theme-store.ts        # 主题状态管理
│   │   ├── itinerary-store.ts    # 行程状态管理
│   │   └── modification-store.ts # 行程修改状态管理
│   ├── config/                   # 配置模块
│   ├── errors/                   # 自定义错误类
│   ├── utils/                    # 工具函数
│   ├── fonts/                    # 字体文件
│   ├── models.ts                 # AI 模型配置
│   ├── encryption.ts             # AES-256 加密工具
│   ├── share.ts                  # 分享功能工具
│   ├── offline.ts                # IndexedDB 离线数据管理
│   ├── sync.ts                   # 数据同步引擎
│   ├── coordinate-converter.ts   # WGS84 <-> GCJ-02 坐标转换
│   ├── geo-clustering.ts         # 地理位置聚类优化
│   ├── map-markers.ts            # 地图标记工具
│   ├── ui-helpers.ts             # UI 辅助函数
│   └── analytics.ts              # 分析工具
├── types/                        # TypeScript 类型
│   ├── index.ts                  # 核心类型定义
│   ├── supabase.d.ts             # Supabase 表类型
│   ├── amap.d.ts                 # 高德地图类型声明
│   └── expense.ts                # 费用相关类型
├── __tests__/                    # 单元和集成测试
│   ├── setup.ts                  # Vitest 全局配置
│   ├── lib/                      # 库函数测试
│   ├── api/                      # API 路由测试
│   ├── integration/              # 集成测试
│   ├── mocks/                    # 测试 Mock
│   └── utils/                    # 测试辅助函数
├── e2e/                          # Playwright E2E 测试
│   ├── helpers.ts                # E2E 辅助函数
│   ├── create-trip.spec.ts       # 创建行程测试
│   ├── export-pdf.spec.ts        # PDF 导出测试
│   └── share-trip.spec.ts        # 分享行程测试
├── docs/                         # 项目文档（40+）
│   ├── README.md                 # 文档索引
│   ├── QUICK_START.md            # 快速启动指南
│   ├── TESTING_GUIDE.md          # 测试指南
│   ├── E2E_TESTING.md            # E2E 测试指南
│   └── ...                       # 其他技术文档
├── scripts/                      # 脚本工具
│   ├── health-check.js           # 健康检查脚本
│   ├── dev-safe.sh               # 安全启动（Linux/Mac）
│   └── dev-safe.bat              # 安全启动（Windows）
├── database/                     # 数据库脚本
│   ├── init.sql                  # 统一数据库初始化脚本 (v3.1)
│   └── migrations/               # 数据库迁移脚本
│       ├── 20251206_add_chat_tables.sql      # 对话功能表
│       ├── 20251206_add_workflow_interrupts.sql  # HITL 中断表
│       ├── add_templates_table.sql           # 旅行模板表
│       └── add_trip_generation_history.sql   # 行程生成历史表
├── public/                       # 静态资源
│   ├── manifest.json             # PWA manifest 文件
│   └── icons/                    # PWA 应用图标
├── vitest.config.ts              # Vitest 单元测试配置
├── playwright.config.ts          # Playwright E2E 测试配置
├── ecosystem.config.js           # PM2 配置文件
├── .env.example                  # 环境变量模板
├── mcp-servers/                  # MCP Server 独立项目
│   └── amap/                     # 高德地图 MCP Server
│       ├── src/                  # 源代码
│       │   ├── tools/            # MCP 工具（天气、POI、路线等）
│       │   └── server.ts         # MCP Server 主类
│       └── tests/                # 单元测试
```

## 🗄️ 数据库结构

### profiles 表
用户配置信息（扩展 Supabase auth.users）
- 基本信息：email、name、avatar_url
- 主题设置：theme (light/dark/system)
- 默认偏好：default_model、default_budget、default_origin

### trips 表
- 基本信息：origin（出发地）、destination（目的地）、日期、预算、人数
- 行程数据：itinerary (JSONB)
- 状态：draft/planned/ongoing/completed
- 分享：share_token（分享标识）、is_public（是否公开）

### expenses 表
- 分类：accommodation/transportation/food/attractions/shopping/other
- 关联：trip_id
- 字段：amount（金额）、description（描述）、date（日期）

### api_keys 表
- 用户自定义 API Keys（AES-256 加密存储）
- 服务类型：anthropic / deepseek / modelscope / map / voice
- 字段：encrypted_key（加密密钥）、key_prefix（显示前缀）、is_active（是否激活）

### chat_sessions 表
- 对话会话管理
- 字段：user_id、title、context（关联行程等上下文）、created_at、updated_at

### chat_messages 表
- 对话消息存储
- 字段：session_id、role（user/assistant/tool）、content、tool_calls、tool_results

### workflow_interrupts 表
- HITL 工作流中断状态存储
- 字段：workflow_id、interrupt_type、state_snapshot、user_decision、resolved_at

### templates 表
- 旅行模板存储
- 字段：user_id、name、description、origin、destination、duration、budget、preferences
- 支持从已有行程创建模板

### trip_history 表
- 行程生成历史记录
- 字段：user_id、trip_id、input_params、result_summary、created_at
- 用于查看和恢复历史版本

详细的数据库设置请查看 [docs/DATABASE_SETUP.md](docs/DATABASE_SETUP.md)

## 🚀 部署

### Vercel 部署（推荐）

1. 将代码推送到 GitHub
2. 在 [Vercel](https://vercel.com) 导入项目
3. 配置环境变量
4. 部署完成

详细步骤请查看 [DEPLOYMENT.md](DEPLOYMENT.md)

## 📱 PWA 和离线功能

### 安装 PWA

应用支持安装到设备主屏幕，像原生应用一样使用。

**手机端（Android/iOS）**：
1. 使用 Chrome/Safari 访问应用
2. 点击页面顶部的"安装 AI 旅行规划"横幅
3. 或通过浏览器菜单选择"添加到主屏幕"

**桌面端（Chrome/Edge）**：
1. 访问应用
2. 点击地址栏的安装图标
3. 确认安装

### 使用离线功能

**准备离线使用**（需要网络）：
1. 登录账号
2. 访问您想离线查看的行程
3. 等待右下角显示"已同步"

**离线访问**（无需网络）：
- ✅ 查看已缓存的行程详情
- ✅ 编辑行程信息
- ✅ 查看费用记录
- ✅ 查看地图（需事先加载）

**数据同步**：
- 网络恢复后自动同步离线编辑
- 右下角显示同步状态和进度
- 支持手动触发同步

### 缓存管理

在 Dashboard 点击"缓存"按钮可以：
- 查看缓存统计
- 清除本地缓存
- 刷新数据

### 详细文档

- **技术实现**: [docs/PWA_IMPLEMENTATION.md](docs/PWA_IMPLEMENTATION.md)
- **用户指南**: [docs/OFFLINE_USAGE.md](docs/OFFLINE_USAGE.md)

## ❓ 常见问题

### 语音输入不可用？
- 确保使用 HTTPS 或 localhost
- 使用支持的浏览器（Chrome/Edge/Safari）
- 授权麦克风权限

### AI 生成失败？
- 检查 API Key 是否正确
- 确认 API 配额充足
- 查看服务器日志获取详细错误

### 数据库保存失败？
- 确保运行了 `database/init.sql` 初始化脚本
- 检查 RLS 策略是否启用
- 确认用户已登录

### 地图无法显示？
- 检查是否配置了 `NEXT_PUBLIC_MAP_API_KEY`
- 确认 API Key 是否正确
- 查看 [docs/MAP_INTEGRATION.md](docs/MAP_INTEGRATION.md) 获取详细配置说明

### 端口被占用或僵尸进程？
- 使用 `npm run dev:check` 检查健康状态
- 使用 `npm run cleanup` 自动清理僵尸进程
- 推荐使用 `npm run dev:safe` 启动，自动处理端口问题
- 查看 [docs/PROCESS_MANAGEMENT.md](docs/PROCESS_MANAGEMENT.md) 获取详细说明

## 📝 更新日志

查看 [CHANGELOG.md](docs/CHANGELOG.md) 了解版本更新历史。

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📄 许可证

MIT License

---

**注意**: 本项目处于活跃开发中。生产环境部署前请确保完成安全审计和性能测试。
